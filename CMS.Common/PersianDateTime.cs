...
namespace CMS
{
    ...
    public class PersianDateTime
    {
        ...
        public static int ToPersianDate()
        {
            return new PersianDateTime(DateTime.Now).ToString(PersianDateTimeFormat.Dateint).To<int>();

        }
        public static int ToPersianDate(DateTime date)
        {
            return new PersianDateTime(date).ToString(PersianDateTimeFormat.Dateint).To<int>();

        }
        public static string ToPersianDate(int date)
        {
            return new PersianDateTime(date).ToString(PersianDateTimeFormat.LongDate);

        }

        public static int ToPersianDateNow()
        {
            return new PersianDateTime(DateTime.Now).ToString(PersianDateTimeFormat.Dateint).To<int>();
        ...
        public static string ToPersianDateLong()
        {
        ...
        public static string TimeSpent(DateTime dateInsert)
        {
            ...
            if (date.Days >= 7)
            {
                    ...
                    timestr += new PersianDateTime(dateInsert).ToString(PersianDateTimeFormat.MinDate);
            }
            if (date.Days >= 1)
            {
                timestr += date.TotalDays.To<int>() + " روز پیش ";
            }
            else if (date.Days < 1 && date.Hours >= 1)
            {
                timestr += (date.Hours).ToString() + " ساعت ";
                timestr += "قبل";
            }
            else if (date.Hours < 1 && date.Minutes >= 10)
            {
        ...
        public static string TimeSpentComment(DateTime dateInsert)
        {
            ...
            if (date.Days >= 30)
            {
                timestr = ToPersianDateLong(dateInsert);
            }
        ...
        /// <returns></returns>
        public static string ToPersianDateNormal(int dateValue)
        {
            if (dateValue.ToString().Length != 8)
            {
                new Exception("Length not is 8");
                return "0";
            }
            return string.Format("{0}/{1}/{2}", dateValue.ToString().Substring(0, 4), dateValue.ToString().Substring(4, 2), dateValue.ToString().Substring(6, 2));
        }

        public static int[] FillYear(int start, int end)
        {
        ...
        public static List<string> FillMonthNumber
        {
            get
            {
        ...
        public static List<string> FillDayNumber
        {
            get
            {
                var result = new List<string>();
